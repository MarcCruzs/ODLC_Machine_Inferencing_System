...
parser.add_argument("--height", type=int, required=True, help="The height of the images.")
parser.add_argument("--num_images", type=int, required=True, help="The number of images to generate.")
...
for i in range(args.num_images):
    ...
    small_pixels = np.random.randint(low=0, high=256, size=(args.height // 10, args.width // 10, 3), dtype=np.uint8)
    small_image = cv2.cvtColor(small_pixels, cv2.COLOR_RGB2BGR)

    # Resize the image to create larger square pixels
    image = cv2.resize(small_image, (args.width, args.height), interpolation=cv2.INTER_NEAREST)
